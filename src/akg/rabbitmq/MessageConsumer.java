...
public class MessageConsumer extends IConnectToRabbitMQ {
	...
	/**
	 *  *  * Set the callback for received messages  * @param handler The
	 * callback  
	 */
	 ...
	 * routingKey  * @param routingKey the binding key eg GOOG  
	 */
	public void AddBinding(String routingKey) {
		try {
			mModel.queueBind(mQueue, Constants.EXCHANGE, routingKey);
		} catch (IOException e) {
	...
	public void RemoveBinding(String routingKey) {
		try {
			mModel.queueUnbind(mQueue, Constants.EXCHANGE, routingKey);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
